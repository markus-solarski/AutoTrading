...
def log_cron_run():
    ...
    args_info = " (Args: " + " ".join(sys.argv[1:]) + ")" if len(sys.argv) > 1 else ""
...
def can_open_new_trade(ib):
    ...
    if orders_month >= MAX_ORDERS_PER_MONTH:
        log_cron_event("Abbruch: Monatslimit erreicht (" + str(orders_month) + "/" + str(MAX_ORDERS_PER_MONTH) + ").")
        return False
    open_positions = [p for p in ib.positions() if p.contract.symbol == SYMBOL and p.position != 0]
    if len(open_positions) >= MAX_CONCURRENT_TRADES:
        log_cron_event("Abbruch: Maximale Positionen erreicht (" + str(len(open_positions)) + "/" + str(MAX_CONCURRENT_TRADES) + ").")
        return False
    return True
...
def place_short_put_with_tp(ib, contract, bid_price):
    ...
    if not trade.isDone():
        ...
        ib.sleep(1)
        log_cron_event("Order nicht innerhalb von " + str(CLOSE_ORDER_WAIT_SECONDS) + "s ausgefuehrt. Storniert.")
        return False
    fill_price = trade.orderStatus.avgFillPrice
    now_str = datetime.datetime.now(BOT_TIMEZONE).strftime("%Y-%m-%d %H:%M:%S")
    formatted_exp = format_expiry(contract.lastTradeDateOrContractMonth)
    with open(LOG_FILE, "a") as f:
        f.write("[" + now_str + "] ORDER_PLACED " + contract.localSymbol + " Expiry: " + formatted_exp + " Strike: " + str(contract.strike) + " Fill: " + str(fill_price) + "\n")
    log_cron_event("Ausgefuehrt: Short Put " + contract.localSymbol + " zu " + format(fill_price, ".2f") + " USD")
    tp_price = round(fill_price * TAKE_PROFIT_FRACTION, 2)
    tp_order = LimitOrder("BUY", 1, tp_price)
    ib.placeOrder(contract, tp_order)
    log_cron_event("Take-Profit platziert: Limit Buy " + contract.localSymbol + " zu " + format(tp_price, ".2f") + " USD")
    return True
...
def main():
    ...
    try:
        ...
        log_cron_event("Mit IB verbunden (Host: " + IB_HOST + ", Port: " + str(IB_PORT) + ", ClientID: " + str(IB_CLIENT_ID) + ")")
        ...
        log_cron_event("Unerwarteter Fehler: " + str(e))
    finally:
        if ib.isConnected():
            ib.disconnect()
