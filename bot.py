...
# Basis-Asset (iShares MSCI Emerging Markets ETF)
SYMBOL = "EEM"
CURRENCY = "USD"
EXCHANGE = "SMART"
...
def run_bot():
    ...
    try:
        ...
        # Verzögerte Marktdaten anfordern (Type 3)
        ...
        # Strike-Auswahl (nächster Strike <= Zielpreis)
        valid_strikes = [s for s in chain.strikes if s <= calculated_target]
        ...
        put_option = Option(SYMBOL, expiry, target_strike, 'P', chain.exchange, currency=CURRENCY)
        ...
        # Bis zu 8 Sekunden warten, bis Bid/Ask aus dem Netzwerk eintreffen
        for _ in range(8):
            ib.sleep(1)
            if ticker.bid > 0 or ticker.ask > 0 or ticker.close > 0:
                break
        ...
        if bid and ask and bid > 0 and ask > 0:
            ...
            print("Verzögerte Marktdaten: Keine aktiven Bid/Ask-Kurse (Fallback verwendet)")
            
        print(f"-> Berechnete Ziel-Prämie (Mid-Preis/Limit): {target_price:.2f} USD")
